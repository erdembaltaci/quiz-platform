...
import DeleteIcon from '@mui/icons-material/Delete'; // Bu icon kullanıldığı için import edildi
...
function AdminPanel() {
  ...
  // Geçici ID sayaçları (modal içindeki sorular için)
  let nextEditOptionTempId = 1; 
  let nextEditQuestionTempId = 1;


  // Soru metni güncelleme (useCallback ile optimize edildi ve bağımlılıkları eklendi)
  const handleEditQuestionTextChange = useCallback((idx, value) => {
  ...
  // Seçenek metni güncelleme (useCallback ile optimize edildi ve bağımlılıkları eklendi)
  ...
  // Doğru cevap seçeneği güncelleme (useCallback ile optimize edildi ve bağımlılıkları eklendi)
  const handleEditCorrectChange = useCallback((qIndex, optionId) => {
    setEditQuestions(prev => {
      const updated = [...prev];
  ...
  // Yeni soru ekle (useCallback ile optimize edildi ve bağımlılıkları eklendi)
  const addEditQuestion = useCallback(() => {
    setEditQuestions(prev => {
      const newOptions = [
        { id: Date.now() + Math.random(), option_letter: 'A', option_text: '' },
        { id: Date.now() + Math.random(), option_letter: 'B', option_text: '' },
        { id: Date.now() + Math.random(), option_letter: 'C', option_text: '' },
        { id: Date.now() + Math.random(), option_letter: 'D', option_text: '' }
      ];
      return [...prev, { 
        ...
        options: newOptions, 
        correct_answer_option_id: null 
      }];
    });
  }, []);

  // Soru sil (useCallback ile optimize edildi ve bağımlılıkları eklendi)
  ...
  useEffect(() => {
    // Sadece admin ise API çağrılarını yap
    ...
    fetchData();
  }, [tab, user]); // user bağımlılığa eklendi
  ...
  const handleAdminEdit = (quiz) => {
    ...
    // ID ataması addEditQuestion içinde yapılacak
